...
vol[30,30,30,] # Este número representa o sinal BOLD neste voxel que representa uma
...
# Gera mapa pra cada voxel

beta1 <- array(0, c(dim(vol)[1], dim(vol)[2], dim(vol)[3], 1))
beta2 <- array(0, c(dim(vol)[1], dim(vol)[2], dim(vol)[3], 1))
pvalor1 <- array(0,c(dim(vol)[1], dim(vol)[2], dim(vol)[3], 1))
pvalor2 <- array(0, c(dim(vol)[1], dim(vol)[2], dim(vol)[3], 1))
...
for (xi in 1:(dim(vol)[1])){
        for (yi in 1:(dim(vol)[2])){
                for (zi in 1:(dim(vol)[3])){
                        ...
                        pvalor1[xi, yi, zi, 1] <- summary(glm)$coef[2,4]
                        pvalor2[xi, yi, zi, 1] <- summary(glm)$coef[3,4]
                }
        }
}
# Aplicar um limiar pela significância estatística
beta1[which(pvalor1 > 0.001)] <-  0
beta2[which(pvalor2 > 0.001)] <-  0

f.write.analyze(beta1, "MapaBeta1",pixdim=c(4,4,4), originator=c(23.5, 32.5,19,0,0))
f.write.analyze(beta2, "MapaBeta2",pixdim=c(4,4,4), originator=c(23.5, 32.5,19,0,0))


######
###### CONTRASTES
# bold[t] <- a + b1*congc[t] + b2*incongc[t] + e[t]

#[a      b1      b2]
#[0      1       0 ] # Ativação da congruente 
#[0     -1       0 ] # Deativação da congruente
#[0      0       1 ] # Ativação da incongruente
#[0      0      -1 ] # Deativação da incongruente
#[0     -1       1 ] # Incongruente ativa mais que a congruente
#[0      1      -1 ] # Congruente ativa mais que a incongruente